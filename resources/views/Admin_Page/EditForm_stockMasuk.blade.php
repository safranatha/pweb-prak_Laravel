            ...
            <div id="main-content" class="col-md-9">
                ...
                <!-- end of navbar -->
                    ...
                    <div class="container w-50 d-flex flex-column justify-content-center gap-5 bg-light mt-5 mb-5">
                        <h1 class="text-center mt-5">Edit Stok Barang Masuk</h1>
                        <form action="{{ route('Updatebarang', $barang->id) }}" id="formBarang" method="POST"
                            class="d-flex flex-column gap-4">
                            @csrf
                            <label for="Id_Barang"> Nama_Barang<br>
                                <input id="Id_Barang" value="{{ $barang->Nama_barang }}" name="Nama_barang"
                                    class="w-100" type="text" required>
                            </label>
                            <label for="k_Barang">Kategori Barang<br>
                                <input id="Id_Barang" value="{{ $barang->Kategori_barang }}" name="Kategori_barang"
                                    class="w-100" type="text" required>
                            </label>
                            <label for="j_Barang"> Jumlah Barang<br>
                                <input id="Id_Barang" value="{{ $barang->Jumlah_barang }}" name="Jumlah_barang"
                                    class="w-100" type="number" required>
                            </label>
                            <label for="t_Barang">Tanggal<br>
                                <input id="Id_Barang" value="{{ $barang->Tanggal }}" name="Tanggal"
                                    class="w-100" type="date" required>
                            </label>

                            <label for="s_Barang">Supplier<br>
                                <input id="Id_Barang" value="{{ $barang->Supplier }}" name="Supplier"
                                    class="w-100" type="text" required>
                            </label>
